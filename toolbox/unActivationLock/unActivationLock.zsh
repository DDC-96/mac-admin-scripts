...
##############################################################
# Messaging
dialogTitle="Sign out of iCloud"
dialogMessage="This company device is currently locked to your iCloud account. Please sign out if you are logged in. IT will be revoking signing with personal accounts moving forward. Thank you."
appIcon="/Applications/Self Service.app/Contents/Resources/AppIcon.icns" #Path to app icon for messaging (optional)
# SwiftDialog Options
swiftDialogOptions=(
  --mini
  --ontop
...
UserDialog (){
  ...
  # Add the action button command to open the specified URL 
    "$dialogPath" --title "$dialogTitle" --message "$dialogMessage" --button1 "Sign Out" --button2 "Cancel" --buton1-action "open x-apple.systempreferences:com.apple.systempreferences.AppleIDSettings" ${swiftDialogOptions[@]} ${iconCMD[@]} 
